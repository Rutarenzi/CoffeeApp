  ...
  persist(
    (set,get) => ({
      ...
      addToFavoriteList: (type:string,id:string)=>
        ...
        set(produce(state=>{
            for(let i=0; i< state.CartList[i]?.prices.length;i++){
        ...
        addToOrderHistoryListFromCart: ()=>set(produce(state =>{
          let temp = state.CartList.reduce(
            (accumulator: number,currentValue: any)=> accumulator + parseFloat(currentValue.ItemPrice),
            0,
          );
        ...
        clearStore: async () => {
          try {
            // Clear the data in AsyncStorage associated with 'Coffee-App'
            await AsyncStorage.removeItem('Coffee-App');
